...
This is a DarkEdif port that does not require Visual Studio Runtime Redistributable, or SDL.dll.

This build instead makes use of a static build of SDL, limited to just including SDL_JOYSTICK, SDL_HAPSTIC (and from necessity as DirectInput expects a video window handle, SDL_VIDEO).
...
* Under RENDER category, enable SOL_STATIC, SDL_JOYSTICK, SDL_HAPTIC, SDL_VIDEO, SDL_LOADSO. Disable the rest.
* Under SDL category, enable SOL_STATIC, SDL_JOYSTICK, SDL_HAPTIC, SDL_VIDEO, SDL_LOADSO. Disable the rest.
* Under VIDEO category, only enable VIDEO_DUMMY.
* Under Ungrouped, enable LIBC.
...
8. In the project, go to "build\include\SDL_config.h", and where `#define HAS_WCSLCPY 1` and `#define HAS_WCSLCAT 1` are, **comment them out**.
...
The resulting files:
* Copy include files from source\include -> repo\DarkEdif\Inc\Shared\SDL.
* Replace source\build\include\SDL_config.h -> repo\DarkEdif\Inc\Shared\SDL\SDL_config.h.
* Copy lib files from source\build\Debug -> repo\DarkEdif\Lib\Windows
* Copy lib files from source\build\Release -> repo\DarkEdif\Lib\Windows
10. All done. Next build of the extension will have the latest SDL2.

Note that Extension.h tags multiple more OS libraries in `#pragma comment (lib, "x.lib")` that SDL desires.
