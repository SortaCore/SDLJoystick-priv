...
bool Extension::DeviceIDOK(int devID)
{
	return (devID >= 0 || devID <= NUM_DEVICES - 1);
}
bool Extension::ButtonIDOK(int buttonID)
{
	return (buttonID >= 0 || buttonID <= NUM_BUTTONS - 1);
}
bool Extension::HatIDOK(int buttonID)
{
	return (buttonID >= 0 || buttonID <= NUM_HATS - 1);
}
bool Extension::AxisIDOK(int axisID)
{
	return (axisID >= 0 || axisID <= NUM_AXES - 1);
}
bool Extension::BallIDOK(int ballID) {
	return (ballID >= 0 || ballID <= NUM_BALLS - 1);
}

bool Extension::IsConnected(int devID)
{
	return DeviceIDOK(devID) && SDL_Data[devID].connected;
}
bool Extension::ButtonPressed(int devID, int buttonID)
{
...
bool Extension::ButtonHeldDown(int devID, int buttonID)
{
	return DeviceIDOK(devID) && ButtonIDOK(buttonID) && SDL_Data[devID].held_buttons[buttonID];
}
bool Extension::ButtonReleased(int devID, int buttonID)
{
	return DeviceIDOK(devID) && ButtonIDOK(buttonID) && (!SDL_Data[devID].held_buttons[buttonID] && SDL_Data[devID].held_buttons_last[buttonID]);
}
bool Extension::AnyButtonPressed(int devID)
{
	if (!DeviceIDOK(devID))
		return false;
...
bool Extension::AnyButtonHeldDown(int devID)
{
	if (!DeviceIDOK(devID))
		return false;
	for (int i = 0; i < NUM_BUTTONS; i++)
...
bool Extension::AnyButtonReleased(int devID)
{
	if (!DeviceIDOK(devID))
		return false;
	for (int i = 0; i < NUM_BUTTONS; i++)
